...
namespace Flx.Delivery.Application.Microservices.Commands.CourierGaveOrderToCustomerUpdateOrderStatusCommand
{
    public sealed class Handler : IRequestHandler<Command, Unit>
    {
        ...
        public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var order = await _orderStorage.Pick(e => e.CourierId == courierUser.Id && e.Status == OrderStatus.CourierGoesToCustomer);

            if (order is null)
            {
                throw new NotExistsDeliveryException($"order not exists or the order status does not match 'CourierGoesToCustomer'");
            }
